...
class Initiative extends Component<{}, InitiativeState> {
  ...
  addCharacter(character: CharacterData) {
    let characters = this.state.characters.concat(character);
    ...
    const characters = this.state.characters
      .map((character, i) => {
        if (character.index === index) {
          ...
          if (i > 0) {
            prevSortDiff =
              this.state.characters[i - 1].sortIndex - character.sortIndex;
          }
  ...
  nextTurn() {
    ...
    let index = this.state.characters.findIndex(
      (character: CharacterData) => character.index === turn
    );
    if (index === -1) {
      index = 0;
    } else {
